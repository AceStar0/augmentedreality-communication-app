...
public class AnimationHandCode : MonoBehaviour
{
    ...
    void Start()
    {
        lines = System.IO.File.ReadLines("Assets/AnimationHandFile.txt").ToList();
    }
    ...
    void Update()
    {
        ...
        for (int i =0; i<=467;i++)
        {
            float x = float.Parse(points[0 + (i * 3)], System.Globalization.CultureInfo.InvariantCulture) * 9;
            float y = -float.Parse(points[1 + (i * 3)], System.Globalization.CultureInfo.InvariantCulture) * 9;
            float z = float.Parse(points[2 + (i * 3)], System.Globalization.CultureInfo.InvariantCulture) * 3;
            Face[i].transform.localPosition = new Vector3(x, y, z);
        }
